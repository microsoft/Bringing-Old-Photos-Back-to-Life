...
class Visualizer:
    def __init__(self, name, checkpoints_dir, results_dir, batchSize, label_nc, isTrain=False, display_winsize=512,
                 tf_log, tensorboard_log):
        
        self.isTrain = isTrain
        self.label_nc = label_nc
        self.batchSize = batchSize
        self.tf_log = isTrain and tf_log
        ...
        if self.tensorboard_log:
            ...
            if self.isTrain:
                self.log_dir = os.path.join(checkpoints_dir, name, "logs")
                if not os.path.exists(self.log_dir):
                    os.makedirs(self.log_dir)
                self.writer = SummaryWriter(log_dir=self.log_dir)
            else:
                print("hi :)")
                self.log_dir = os.path.join(checkpoints_dir, name, results_dir)
                if not os.path.exists(self.log_dir):
                    os.makedirs(self.log_dir)
    ...
    def display_current_results(self, visuals, epoch, step):
        ...
        if self.tensorboard_log:
            ...
            img_grid = vutils.make_grid(output, nrow=self.batchSize, padding=0, normalize=False)
    ...
    def convert_visuals_to_numpy(self, visuals):
        for key, t in visuals.items():
            tile = self.batchSize > 8
            if "input_label" == key:
                t = util.tensor2label(t, self.label_nc + 2, tile=tile)  ## B*H*W*C 0-255 numpy
            else:
